...
echo 2. This setup will scan camera numbers.
echo 3. It also saves preview images for available camera numbers.
...
echo.
set /p CAMERA_INDEX=Type the camera number to use, then press Enter: 
if "%CAMERA_INDEX%"=="" set CAMERA_INDEX=0
echo.
echo Saving camera number %CAMERA_INDEX%...
...
echo If camera_ok is false, run this setup again and choose another available number.
echo.
pause
